
login_dict = {
    "Login_WindowTitle": ['PBX login screen', 'Вход в интерфейс управления АТС'],
    "show_demo_pushButton": ["Show demo", "Демо"],
    "login_button": ["Login", "Войти"],
    "app ID": ["Login", "Логин"],
    "app Secret": ["Password", "Пароль"],
    "languag": ["Lang", "Язык"],
    "app credentials": ["Please, insert your credentials:", "Пожалуйста, введите ваши данные:"],
    "login warning": ['Login incorrect', 'Указаны некорректные данные!'],
    "app credentials wrong": ["Please, check your credentials.\nCredentials are wrong",
                              "Введенные данные не корректны!\nПроверьте введенные данные!"],
    "error frame name": ['WARNING!!!', 'ВНИМАНИЕ!!!'],
    "no credentials": ["Please, check your credentials!", "Введенные данные не корректны!"]
}

menu_dict = {
    "Menu_WindowTitle": ["PBX functions and settings menu", "Меню функций и настроек АТС"],
    "Extensions": ["Extensions (SIP ID)", "Добавочные (SIP ID)"],
    "Routing": ["Routing and numbers (DIDs)", "Маршрутизация и Номера (DIDs)"],
    "CDR": ["CDR and call recordings", "Статистика и записи разговоров"],
    "Queue_Group": ["Queues (groups)", "Очереди (группы)"],
    "conf": ["Conference", "Конференция"],
    "DialRul": ["Dial Rules", "Правила набора"],
    "Faxes": ["Faxes and voicemails", "Факсы и голосовая почта"],
    "languag": login_dict["languag"],
    "uptitle": ["NONE", "NONE"],
    "Settings": ['Settings', 'Настройки'],
    "passwd": ['Change password', 'Сменить пароль'],
    "email": ['Change contact e-mail', 'Сменить контактный почтовый адрес'],
    "timeZ": ['Change time zone', 'Сменить временную зону'],
    "notif": ['Notification settings', 'Настройки уведомлений'],
    "white and black list": ['Black|White list', 'Черный|Белый список'],
    "sounds": ['Sounds management', 'Управление звуковыми файлами'],
    "Hello": ['Hello, ', 'Здравствуйте, '],
    "domain": ['Your domain: ', 'Ваш домен: '],
    'prefix': ['Your SIP ID prefix: ', 'Ваш SIP ID всегда начинается с ']
}

ext_dict = {
    "ext_menu_window": ["Phone extensions", "Телефонные добавочные"],
    "create_ext": ["Create extensions", "Создать добавочный"],
    "unused": ["Show all unused extensions", "Показать неиспользуемые добавочные"],
    "label": ["Label|Caller ID name", "Имя звонящего"],
    "inccallR": ["Incoming call rules", "Правила для входящего звонка"],
    "termpwd": ["Change SIP ID password", "Изменить пароль этого SIP ID"],
    "extnum": ["Set external number", "Закрепить внешний номер"],
    "record": ["Call record setting", "Настройка записи разговора"],
    "oth_sett": ["Other settings", "Остальные настройки"]
}